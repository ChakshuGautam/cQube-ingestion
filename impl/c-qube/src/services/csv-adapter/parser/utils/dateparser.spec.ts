...
describe('DateParser', () => {
  ...
  test('should parse date correctly with format 1 and timezone', async () => {
    const date = await parser1.parseDate('02/01/23');
    expect(date).toEqual(new Date('2023-01-02T00:00:00.000Z'));
  });

  test('should parse date correctly with format 2 and no timezone', async () => {
    const date = await parser2.parseDate('02-01-2023');
    expect(date).toEqual(new Date('2023-01-02T00:00:00.000Z'));
  });
